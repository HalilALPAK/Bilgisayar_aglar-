...
def run_client(filename, loss_rate, timeout):
    ...
    for seq, chunk in enumerate(chunks):
        ...
        while retries <= MAX_RETRIES:
            try:
                print(f"Sending Packet {seq} (attempt {retries + 1}/{MAX_RETRIES + 1})...")
        ...
        if not acked:
            failed_packets += 1
            log_event("FAILED", "DATA", seq)
            print(f"Packet {seq} FAILED after {MAX_RETRIES} retries. Skipping.")
    ...
    if failed_packets > 0:
        print(f"WARNING: {failed_packets} packet(s) could not be delivered after {MAX_RETRIES} retries.")

    # Print MD5 of original file for integrity comparison with server
    import hashlib
    with open(filename, "rb") as f:
